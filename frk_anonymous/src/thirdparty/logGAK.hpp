 ...
 * Added log1p function for windows platforms. Uncomment lines 62-66 if you are compiling this file with matlab on
 windows.
 *
 * Previous versions:
...
inline double logGAK(double* seq1, double* seq2, int nX, int nY, int dimvect, double sigma, int triangular)
// /* Implementation of the (Triangular) global alignment kernel.
//  *
//  * See details about the matlab wrapper mexFunction below for more information on the inputs that need to be called
//  from
//  * Matlab
//  *
//  * /* seq1 is a first sequence represented as a matrix of real elements. Each line i corresponds to the vector of
//  * observations at time i. seq2 is the second sequence formatted in the same way. nX, nY and dimvect provide the
//  number
//  * of lines of seq1 and seq2. sigma stands for the bandwidth of the \phi_\sigma distance used kernel lambda is an
//  * additional factor that can be used with the Geometrically divisible Gaussian Kernel triangular is a parameter
//  which
//  * parameterizes the triangular kernel kerneltype selects either the Gaussian Kernel or its geometrically divisible
//  * equivalent */

{
    ...
    double sum = 0;
    double gram, Sig;
    /* logM is the array that will stores two successive columns of the (nX+1) x (nY+1) table used to compute the final
     * kernel value*/
